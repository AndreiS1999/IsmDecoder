...
static const RCSwitch::Protocol PROGMEM proto[] = {
  ...
  { 500, {  0, 8  }, {  1,  2 }, {  1,  4 }, false },
  { 500, {  0, 18 }, {  1,  4 }, {  1,  8 }, false }
...
int RCSwitch::nReceiveTolerance = 130;
...
void RCSwitch::enableReceiveExtended(int interrupt) {

  
  this->nReceiverInterrupt = interrupt;
  this->enableReceiveExtended();
}

void RCSwitch::enableReceiveExtended() {
  if (this->nReceiverInterrupt != -1) {
    RCSwitch::nReceivedValue = 0;
    RCSwitch::nReceivedBitlength = 0;
#if defined(RaspberryPi) // Raspberry Pi
    wiringPiISR(this->nReceiverInterrupt, INT_EDGE_BOTH, &handleInterrupt);
#else // Arduino
    attachInterrupt(this->nReceiverInterrupt, handleInterruptExtended, CHANGE);
#endif
  }
}
...
void RECEIVE_ATTR RCSwitch::handleInterruptExtended() {
  ...
  if (duration > 4700) {
    ...
    if (diff(duration, RCSwitch::timings[0]) < 200) {
      ...
      repeatCount++;
      if (repeatCount == 2) {
        for(unsigned int i = 3; i <= numProto; i++) {
          if (receiveProtocol(i, changeCount)) {
            // receive succeeded for protocol i
            break;
          }
